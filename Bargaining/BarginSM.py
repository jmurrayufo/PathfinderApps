...
class BargainSM(object):
   ...
   def Step0( self ):
      ...
      Buyer.SkillsList = np.random.choice( Funcs.SkillChecks( self.Seller.Level ), 3, replace = True,  p = tmpProbSet )
      Buyer.BluffSkill = Buyer.SkillsList[0]
      Buyer.AppraiseSkill = Buyer.SkillsList[1]
      Buyer.SenseMotiveSkill = Buyer.SkillsList[2]

      Buyer.CHAModifier = np.random.choice( Funcs.Attribute( self.Seller.Level ), p = tmpProbSet )
